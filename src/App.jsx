...
import { Target, TrendingDown, Wrench, AlertTriangle, BookOpen, BrainCircuit, Activity } from 'lucide-react';
...
const convergentData = [
  ...
  { name: 'Agro-Fishery', Desired: 0, Aptitude: 0, Full: 0 },
];

const humssData = [
  { name: 'Desired (A)', Value: 44.5 },
  { name: 'Aptitude Match (B)', Value: 42.6 },
  { name: 'Interest Match (C)', Value: 14.7 },
...
const divergentData = [
  ...
  { name: 'Arts & Design', Desired: 5.7, Aptitude: 33.2 },
  { name: 'Sports', Desired: 17.0, Aptitude: 40.8 },
  { name: 'Home Econ.', Desired: 3.4, Aptitude: 44.9 },
...
const stemData = [
  { name: 'Desired Track', Desired: 47.9, Aptitude: 0, Interest: 0 },
  { name: 'Aptitude-Matched', Desired: 0, Aptitude: 3.0, Interest: 0 },
  { name: 'Interest-Matched', Desired: 0, Aptitude: 0, Interest: 7.9 },
];

const gasData = [
  { name: 'Desired Track', Desired: 1.5, Aptitude: 0, Interest: 0 },
  { name: 'Aptitude-Matched', Desired: 0, Aptitude: 56.2, Interest: 0 },
  { name: 'Interest-Matched', Desired: 0, Aptitude: 0, Interest: 30.6 },
];

const untappedData = [
  { name: 'Sports Track', Desired: 17.0, Aptitude: 40.8 },
  { name: 'TVL: Agro-Fishery', Desired: 1.9, Aptitude: 49.1 },
];
...
const mathSectionData = [
  { name: 'Section A', 'Not Met': 6, 'Progressing': 68, 'Proficient': 21, 'Highly Proficient': 6 },
  { name: 'Section B', 'Not Met': 6, 'Progressing': 45, 'Proficient': 39, 'Highly Proficient': 10 },
  { name: 'Section C', 'Not Met': 9, 'Progressing': 48, 'Proficient': 39, 'Highly Proficient': 3 },
  { name: 'Section D', 'Not Met': 10, 'Progressing': 39, 'Proficient': 48, 'Highly Proficient': 3 },
];

const sciSectionData = [
  { name: 'Section A', 'Not Met': 0, 'Progressing': 35, 'Proficient': 56, 'Highly Proficient': 9 },
  { name: 'Section B', 'Not Met': 0, 'Progressing': 42, 'Proficient': 55, 'Highly Proficient': 3 },
  { name: 'Section C', 'Not Met': 3, 'Progressing': 42, 'Proficient': 55, 'Highly Proficient': 0 },
  { name: 'Section D', 'Not Met': 0, 'Progressing': 48, 'Proficient': 48, 'Highly Proficient': 3 },
];
...
const CustomLabel = (props) => {
  ...
  return (
    <text x={x + width / 2} y={y - 10} fill="#333" fontSize="14" fontWeight="bold" textAnchor="middle">
      {`${value}%`}
    </text>
  );
};
...
export default function App() {
  ...
  const triggerSparks = (e) => {
    ...
    confetti({ particleCount: 60, spread: 70, origin: { x, y }, colors: ['#38bdf8', '#34d399', '#fbbf24'], disableForReducedMotion: true, ticks: 100, gravity: 1.2 });
  };

  // Function to change tabs and scroll to top smoothly
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-800 flex flex-col">
      ...
      {/* ---------------- NAVIGATION BAR ---------------- */}
      <nav className="sticky top-0 z-50 bg-[#1a4331] text-white shadow-lg border-b-[4px] border-[#f6b21c]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex flex-wrap justify-center md:justify-between items-center gap-4">
          <div className="font-serif font-bold text-xl hidden lg:block tracking-tight text-[#f6b21c]">DLSU IS Data Report</div>
          <div className="flex flex-wrap justify-center gap-2 md:gap-4 text-sm font-semibold">
            <button onClick={() => handleTabChange('summary')} className={`px-4 py-2 rounded-md transition-colors ${activeTab === 'summary' ? 'bg-[#f6b21c] text-[#1a4331]' : 'hover:bg-[#2a5a41]'}`}>Executive Summary</button>
            <button onClick={() => handleTabChange('shs')} className={`px-4 py-2 rounded-md transition-colors ${activeTab === 'shs' ? 'bg-[#f6b21c] text-[#1a4331]' : 'hover:bg-[#2a5a41]'}`}>01. SHS Alignment</button>
            <button onClick={() => handleTabChange('sel')} className={`px-4 py-2 rounded-md transition-colors ${activeTab === 'sel' ? 'bg-[#f6b21c] text-[#1a4331]' : 'hover:bg-[#2a5a41]'}`}>02. SEL Profile</button>
            <button onClick={() => handleTabChange('sba')} className={`px-4 py-2 rounded-md transition-colors ${activeTab === 'sba' ? 'bg-[#f6b21c] text-[#1a4331]' : 'hover:bg-[#2a5a41]'}`}>03. Grade 6 SBA</button>
            <button onClick={() => handleTabChange('recs')} className={`px-4 py-2 rounded-md transition-colors ${activeTab === 'recs' ? 'bg-[#f6b21c] text-[#1a4331]' : 'hover:bg-[#2a5a41]'}`}>04. Recommendations</button>
          </div>
        </div>
      </nav>
        ...
        {activeTab === 'summary' && (
          <div className="animate-fade-in">
            ...
            <section className="bg-[#1a4331] text-white p-12 md:p-20 flex flex-col justify-center relative overflow-hidden">
              <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-[#2a5a41] rounded-full opacity-50 pointer-events-none"></div>
              ...
              <div className="relative z-10 max-w-6xl mx-auto w-full py-10">
                <h3 className="text-[#f6b21c] font-bold tracking-[0.2em] text-sm uppercase mb-6">Academic Year 2025–2026</h3>
                <h1 className="text-5xl md:text-7xl font-bold mb-6 font-serif leading-tight">Student Assessment & <br />Development Report</h1>
                ...
                <div className="w-48 h-1 bg-[#f6b21c] mb-20"></div>
                <div>
            ...
            {/* Executive Summary */}
            <section className="p-12 max-w-7xl mx-auto mb-10 animate-fade-in delay-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#f4f7f5] rounded-lg overflow-hidden flex flex-col shadow-sm border border-gray-100 transition-transform duration-300 hover:-translate-y-2">
                  <div className="bg-[#1a4331] text-white text-center py-4 font-bold tracking-wide">Track Alignment (Gr. 9)</div>
                  <div className="p-8 flex-grow flex flex-col justify-between">
                    <div>
                      <h2 className="text-[#1a4331] text-5xl font-bold mb-4">47.9% → 3.0%</h2>
                      <p className="text-gray-600 mb-8 font-medium">wanted STEM, but had matching aptitude</p>
                      <h2 className="text-[#1a4331] text-5xl font-bold mb-4">43.4%</h2>
                ...
                <div className="bg-[#f4f7f5] rounded-lg overflow-hidden flex flex-col shadow-sm border border-gray-100 transition-transform duration-300 hover:-translate-y-2">
                  ...
                  <div className="p-8 flex-grow flex flex-col justify-between">
                    <div>
                      <h2 className="text-[#2a6a5a] text-5xl font-bold mb-4">3.40 – 3.60</h2>
                      <p className="text-gray-600 mb-8 font-medium">Grit's range — the lowest-scoring dimension in every cohort</p>
                      <h2 className="text-[#2a6a5a] text-5xl font-bold mb-4">3.89 → 3.51</h2>
                ...
                <div className="bg-[#f4f7f5] rounded-lg overflow-hidden flex flex-col shadow-sm border border-gray-100 transition-transform duration-300 hover:-translate-y-2">
                  ...
                  <div className="p-8 flex-grow flex flex-col justify-between">
                    <div>
                      <h2 className="text-[#42825f] text-5xl font-bold mb-4">50.4%</h2>
                      <p className="text-gray-600 mb-8 font-medium">of Math students are only "Progressing", not yet Proficient</p>
                      <h2 className="text-[#42825f] text-5xl font-bold mb-4">53.5%</h2>
          ...
          <div className="animate-fade-in pb-20">
            <section className="p-12 max-w-7xl mx-auto bg-gray-50 border-b border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <div><strong className="text-[#1a4331]">A. Desired Track Survey:</strong> Self-reported student preference — what the student wants to pursue.</div>
                <div><strong className="text-[#1a4331]">B. Achievement & Aptitude Tests:</strong> Objective, standardized measures of cognitive competency and subject mastery.</div>
                <div><strong className="text-[#1a4331]">C. Interest & Study Habits Inventory:</strong> Behavioral mapping of natural inclinations, career interest, and success motivators.</div>
              </div>
            </section>
              ...
              <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={convergentData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                    ...
                    <Bar dataKey="Desired" name="Desired Track (A)" fill="#1a4331" animationDuration={1000} />
                    <Bar dataKey="Aptitude" name="Aptitude Match (B)" fill="#42825f" animationDuration={1000} />
                    <Bar dataKey="Full" name="Full Consistency (A+B+C)" fill="#f6b21c" animationDuration={1000} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </section>
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-[#f8faf9]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="bg-[#1a4331] rounded-xl p-10 text-white flex flex-col justify-center shadow-lg">
                  <h2 className="text-8xl font-bold text-[#f6b21c] mb-6 font-serif">12.8%</h2>
                  ...
                  <div className="border-t border-[#347b59] pt-6"><p className="italic text-sm">44.5% desired HUMSS • 42.6% were aptitude-matched</p></div>
                </div>
                <div className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={humssData} margin={{ top: 30, right: 30, left: 0, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                      <XAxis dataKey="name" tick={{ fontSize: 12 }} />
                      <YAxis domain={[0, 50]} />
                      <Bar dataKey="Value" fill="#42825f" barSize={80} animationDuration={1000}>
                        <LabelList dataKey="Value" position="top" fill="#333" fontSize={16} fontWeight="bold" formatter={(val) => `${val}%`} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={divergentData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip cursor={{ fill: '#f4f4f4' }} />
                    <Legend wrapperStyle={{ paddingTop: '20px' }} />
                    <Bar dataKey="Desired" name="Desired Track" fill="#db5a5a" animationDuration={1000} />
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="bg-[#edf5f0] rounded-xl p-10 flex flex-col justify-center">
                  ...
                  <div className="border-t border-gray-300 pt-6"><p className="italic text-sm text-gray-600">The report attributes it to prestige, parental pressure, or perceived economic value rather than measured cognitive readiness.</p></div>
                </div>
              </div>
            </section>
            ...
            <section className="p-12 max-w-7xl mx-auto bg-[#1a4331] text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div className="flex justify-center">
                  <div className="w-80 h-80 rounded-full border-[6px] border-[#f6b21c] flex flex-col items-center justify-center shadow-[0_0_40px_rgba(246,178,28,0.2)]">
                    <h2 className="text-7xl font-bold text-[#f6b21c] font-serif mb-2">43.4%</h2>
                    <p className="text-gray-300 text-sm">of the Grade 9 cohort</p>
                  </div>
                </div>
                <div className="flex flex-col justify-center">
                  ...
                  <ul className="space-y-6 text-gray-300 list-disc pl-5">
                    ...
                    <li>Motivated or not, students may still insist on tracks where they lack technical readiness.</li>
                  </ul>
                </div>
              </div>
            </section>
        ...
        {activeTab === 'sel' && (
          <div className="animate-fade-in pb-20">
            <section className="p-12 max-w-7xl mx-auto bg-gray-50 border-b border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                <div><strong className="text-xl text-[#1a4331]">Grades 1-3</strong><br /><span className="text-sm text-gray-500">301 students</span></div>
                <div><strong className="text-xl text-[#1a4331]">Grades 4-6</strong><br /><span className="text-sm text-gray-500">321 students</span></div>
                <div><strong className="text-xl text-[#1a4331]">Grades 7-9</strong><br /><span className="text-sm text-gray-500">686 students</span></div>
              </div>
            </section>
                ...
                <div className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={selGritData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                      ...
                      <Tooltip />
                      <Legend wrapperStyle={{ paddingTop: '20px' }} />
                      <Line type="monotone" dataKey="Teacher-Student Connection" stroke="#42825f" strokeWidth={3} dot={{ r: 5 }} animationDuration={1500} />
                      <Line type="monotone" dataKey="Physical Surroundings" stroke="#f6b21c" strokeWidth={3} dot={{ r: 5 }} animationDuration={1500} />
                      <Line type="monotone" dataKey="Grit" stroke="#db5a5a" strokeWidth={3} dot={{ r: 5 }} animationDuration={1500} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                <div className="bg-[#edf5f0] rounded-xl p-10 flex flex-col justify-center shadow-sm" onClick={triggerSparks}>
                  <h2 className="text-6xl font-bold text-[#db5a5a] mb-6 font-serif cursor-pointer hover:scale-105 transition-transform">3.40 – 3.60</h2>
                  <p className="text-lg text-gray-800 leading-relaxed mb-6">Grit's range across all cohorts — students rate themselves as only "somewhat focused and persistent" toward long-term goals.</p>
                  <div className="pt-6 border-t border-gray-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="bg-[#edf5f0] rounded-xl p-10 flex flex-col justify-center shadow-sm">
                  ...
                  <ul className="space-y-4 text-gray-800 text-sm list-disc pl-5">
                    ...
                    <li>The intermediate cohort (Grades 4–6) peaks at 4.01, then dips again by Grades 7–9 (3.66).</li>
                  </ul>
                </div>
              </div>
            </section>
              ...
              <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
                <table className="w-full text-sm text-left text-gray-700">
                  <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b border-gray-200">
                    <tr><th className="px-6 py-4">Dimension</th><th className="px-6 py-4">Grades 1-3</th><th className="px-6 py-4">Grades 4-6</th><th className="px-6 py-4">Grades 7-9</th><th className="px-6 py-4">Trend</th></tr>
                  </thead>
                  <tbody>
                    {selTable.map((row, i) => (
                      <tr key={i} className="border-b border-gray-100 hover:bg-[#edf5f0] transition-colors duration-200">
                        <td className="px-6 py-4 font-bold text-gray-900">{row.dim}</td><td className="px-6 py-4">{row.g1}</td><td className="px-6 py-4">{row.g4}</td><td className="px-6 py-4">{row.g7}</td><td className="px-6 py-4 italic">{row.trend}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-[#f8faf9]">
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6">
                  ...
                  <div><h3 className="font-bold text-xl mb-2 text-gray-900">Evidence-Based Grit & Resilience Curriculum</h3><p className="text-gray-600">Embed goal-setting exercises, mistake-normalization strategies, and extended project milestones.</p></div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6">
                  ...
                  <div><h3 className="font-bold text-xl mb-2 text-gray-900">Junior High Transition Support Systems</h3><p className="text-gray-600">Intentional socio-emotional check-ins and peer mentorship to counter the Grades 7–9 dip.</p></div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6">
                  ...
                  <div><h3 className="font-bold text-xl mb-2 text-gray-900">Early Digital Citizenship Frameworks</h3><p className="text-gray-600">Introduce formal internet-safety modules starting in Grades 1–3.</p></div>
                </div>
              </div>
            </section>
          </div>
        ...
        {activeTab === 'sba' && (
          <div className="animate-fade-in pb-20">
              ...
              <div className="h-[500px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={mathSectionData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip cursor={{ fill: '#f4f4f4' }} />
                    <Legend wrapperStyle={{ paddingTop: '20px' }} />
                    <Bar dataKey="Not Met" stackId="a" fill="#db5a5a" animationDuration={1000} />
                    <Bar dataKey="Progressing" stackId="a" fill="#f6b21c" animationDuration={1000} />
                    <Bar dataKey="Proficient" stackId="a" fill="#42825f" animationDuration={1000} />
                    <Bar dataKey="Highly Proficient" stackId="a" fill="#1a4331" animationDuration={1000} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </section>
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-[#f8faf9]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                  <h2 className="text-[#42825f] text-2xl font-bold mb-6">STRENGTHS</h2>
                  <div className="space-y-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Understanding graph types & purposes</h4><p className="text-sm text-gray-600">Score: 94-100% | Nat. bench: 80%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Reading & interpreting charts/tables</h4><p className="text-sm text-gray-600">Score: 87-100% | Nat. bench: 80%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Base & exponent concepts</h4><p className="text-sm text-gray-600">Score: 85-100% | Nat. bench: 87%</p></div>
                  </div>
                </div>
                <div>
                  <h2 className="text-[#db5a5a] text-2xl font-bold mb-6">GAPS</h2>
                  <div className="space-y-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Listing outcomes (probability)</h4><p className="text-sm text-gray-600">Score: 3-10% | Nat. bench: 20%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Corresponding angles on parallel lines</h4><p className="text-sm text-gray-600">Score: 3-13% | Nat. bench: 19%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Calculating circumference of a circle</h4><p className="text-sm text-gray-600">Score: 18-19% | Nat. bench: 29%</p></div>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-[#f8faf9]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                  <h2 className="text-[#42825f] text-2xl font-bold mb-6">STRENGTHS</h2>
                  <div className="space-y-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Interpreting graphs</h4><p className="text-sm text-gray-600">Score: 97-100% | Nat. bench: 92%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Identifying tropical rainforest ecosystems</h4><p className="text-sm text-gray-600">Score: 97-100% | Nat. bench: 86%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Demonstrating basic measurement skills</h4><p className="text-sm text-gray-600">Score: 88-97% | Nat. bench: 81%</p></div>
                  </div>
                </div>
                <div>
                  <h2 className="text-[#db5a5a] text-2xl font-bold mb-6">GAPS</h2>
                  <div className="space-y-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Stating the Law of Conservation of Energy</h4><p className="text-sm text-gray-600">Score: 0-3% | Nat. bench: 30%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Properties of homogeneous mixtures</h4><p className="text-sm text-gray-600">Score: 6-16% | Nat. bench: 23%</p></div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100"><h4 className="font-bold text-gray-900">Explaining the process of digestion</h4><p className="text-sm text-gray-600">Score: 9-15% | Nat. bench: 20%</p></div>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="flex flex-col justify-center space-y-6 bg-[#edf5f0] p-8 rounded-xl shadow-sm">
            ...
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200 bg-[#f8faf9]">
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6 hover:shadow-md">
                  ...
                  <div><h3 className="font-bold text-xl mb-2">Re-anchor Geometric & Spatial Reasoning</h3><p className="text-gray-600">Shift from formula memorization to hands-on manipulation (physical models).</p></div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6 hover:shadow-md">
                  ...
                  <div><h3 className="font-bold text-xl mb-2">Concrete Probability & Outcome Tracking</h3><p className="text-gray-600">Tree diagrams, grid charts, and hands-on outcome experiments.</p></div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6 hover:shadow-md">
                  ...
                  <div><h3 className="font-bold text-xl mb-2">Interactive Physical Science Activities</h3><p className="text-gray-600">Lab experiments and simulations for energy conservation.</p></div>
                </div>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-start gap-6 hover:shadow-md">
                  ...
                  <div><h3 className="font-bold text-xl mb-2">Differentiate by Tier</h3><p className="text-gray-600">Small-group remediation for Not Met Standards; weekly spiral review for the Progressing "middle majority".</p></div>
                </div>
              </div>
            </section>
          </div>
        ...
        {activeTab === 'recs' && (
          <div className="animate-fade-in pb-20">
            <section className="p-12 max-w-7xl mx-auto border-b border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="flex gap-4 p-4 hover:bg-gray-50 rounded-lg"><div className="text-[#42825f] font-bold text-2xl">1</div><div><h4 className="font-bold text-lg mb-2">Environment is a genuine strength</h4><p className="text-sm text-gray-600">High Teacher-Student Connection and Physical Surroundings scores show DLSU IS's foundation is solid.</p></div></div>
                <div className="flex gap-4 p-4 hover:bg-gray-50 rounded-lg"><div className="text-[#42825f] font-bold text-2xl">2</div><div><h4 className="font-bold text-lg mb-2">Desire and readiness don't always meet</h4><p className="text-sm text-gray-600">Grade 9 STEM desire vs. aptitude, and Grade 6 Math's "Progressing" cluster show aspiration outpacing readiness.</p></div></div>
                <div className="flex gap-4 p-4 hover:bg-gray-50 rounded-lg"><div className="text-[#42825f] font-bold text-2xl">3</div><div><h4 className="font-bold text-lg mb-2">The junior high years are the pressure point</h4><p className="text-sm text-gray-600">The SEL "Adolescent Dip" and Grit gap concentrate exactly where students choose SHS tracks.</p></div></div>
                <div className="flex gap-4 p-4 hover:bg-gray-50 rounded-lg"><div className="text-[#42825f] font-bold text-2xl">4</div><div><h4 className="font-bold text-lg mb-2">Abstract reasoning is the shared skill gap</h4><p className="text-sm text-gray-600">Grade 6 Math and Science both show weakness in abstract, multi-step, or unseen-mechanism reasoning.</p></div></div>
              </div>
            </section>

            <section className="p-12 max-w-7xl mx-auto bg-[#1a4331] text-white relative overflow-hidden">
              <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-[#2a5a41] rounded-full opacity-50 pointer-events-none"></div>

              <div className="relative z-10">
                <h3 className="text-[#f6b21c] font-bold tracking-widest text-sm uppercase mb-4">Closing — Consolidated Recommendations</h3>
                <h1 className="text-4xl font-bold mb-12 font-serif">Three Priorities for the Coming Year</h1>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                  <div className="bg-[#2a5a41]/50 p-8 rounded-xl border border-[#3b7b59]/30">
                  ...
                  <div className="bg-[#2a5a41]/50 p-8 rounded-xl border border-[#3b7b59]/30">
                    ...
                    <p className="text-gray-300 leading-relaxed">Hands-on, manipulative-based instruction for geometry, probability, and physical science concepts.</p>
                    <div className="mt-4 pt-4 border-t border-[#3b7b59]/50">
                      <p className="text-sm text-gray-400 italic">Operational Note: To execute this effectively, hardware procurement workflows must be streamlined, ensuring internal vendor proposals are processed without administrative delay.</p>
                    </div>
                  </div>
                </div>
              </div>
